#this is a program to check wether it is changing popularity everyday since people can vote 
#in online there is a possibility to change the most high rated definitions with time.
    ...
    with open("mc_wordpair.csv", "r") as mfile:
        for line in mfile:
            ...
            for d1 in range(len(sentence1)):
                ...
                sim = (sim1 + sim2) / 2.0
                si = [linesp[0],linesp[1],sim]
                out.append(si)
            b = np.array(out)
            today = datetime.datetime.now().date()
            i = 0
            filename = 'outputdata{}'+str(i)+'.csv'.format(today.isoformat())
